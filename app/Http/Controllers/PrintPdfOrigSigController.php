...
class PrintPdfOrigSigController extends Controller {
    ...
    public function export($coeCode){
        ...
        if(Auth::user()->is_admin==1){ 
            if($data['coe']){
                if($data['coe']->type_desc == "CERTIFICATE OF EMPLOYMENT AND COMPENSATION") {
                ...
                }elseif($data['coe']->type_desc == "CERTIFICATE OF EMPLOYMENT AND COMPENSATION ( WITH NOTARY )") {
                    $pdf = PDF::loadView('coe.cec_with_notary_print', $data)->setPaper('Letter');
                    return $pdf->stream('coe.cec_with_notary_print.pdf');
                    ...
                    return "Oopsss! There's something wrong!";
                }
            }else{
            ...
            Crypt::decrypt($coeCode);
            return redirect('coe-details/'.$coeCode);
        }
    ...
    public function coe($coeCode){
        ...
        // ->where('coe.created_by',Auth::user()->email)
        ->where('coe.coe_code', $coeCode) 
